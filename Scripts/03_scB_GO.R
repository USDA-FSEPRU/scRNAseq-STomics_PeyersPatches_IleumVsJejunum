...
AllCellsDGE <- read_xlsx('/home/Jayne.Wiarda/SI_PP_SC_ST/DGE/CellTypeDGE_AllSamples.xlsx')
...
AllCells_results %>% write_tsv('/home/Jayne.Wiarda/SI_PP_SC_ST/GeneOntology/AllCells_GOresults.tsv')

# Create dot plot of selected GO processes for all cell types
terms <- c('positive regulation of B cell activation',
...
AllCells_results <- AllCells_results[AllCells_results$Term %in% terms, ]
AllCells_results$cluster <- factor(AllCells_results$cluster, 
                                   levels=c(levels(seu$celltype)))
AllCells_results$Term <- factor(AllCells_results$Term,
                                levels = c('positive regulation of B cell activation',
                                           'B cell differentiation',
                                           'interleukin-10 production',
                                           'base-excision repair',
                                           'double-strand break repair',
                                           ...
                                           'positive regulation of muscle cell differentiation',
                                           'collagen biosynthetic process'))
AllCells_results$pval <- as.numeric(AllCells_results$pval)
ggplot(AllCells_results) +
...
TissueDGE <- read_xlsx('/home/Jayne.Wiarda/SI_PP_SC_ST/DGE/CellTypeDGE_AllSamples_TissueDEGs.xlsx')

Jgenes <- subset(TissueDGE, avg_log2FC > 0)
Jgenes$cluster <- Jgenes$pop1

Igenes <- subset(TissueDGE, avg_log2FC < 0)
Igenes$cluster <- Igenes$pop2

tissueGenes <- rbind(Jgenes, Igenes)
table(tissueGenes$cluster)
tissueGenes <- tissueGenes[as.numeric(ave(tissueGenes$cluster, 
                                          tissueGenes$cluster, 
                                          FUN=length)) > 2, ] # Choosing to omit gene lists without more than 2 DEGs found for a tissue-specific cell type
...
TissueDGE_results$Fold_enrichment <- TissueDGE_results$Significant/TissueDGE_results$Expected # add in stat for fold enrichment
TissueDGE_results <- subset(TissueDGE_results, Significant > 1) # remove GO terms with only one enriched genedim

TissueDGE_results %>% write_tsv('/home/Jayne.Wiarda/SI_PP_SC_ST/GeneOntology/CellTypeTissueDGE_GOresults.tsv')

# Create dot plot of selected GO processes for jejunal vs ileal B cells
terms <- c('antigen processing and presentation',
           'antigen processing and presentation of peptide or polysaccharide antigen via MHC class II',
           'B cell activation',
           'T cell activation involved in immune response',
           'humoral immune response',
           'humoral immune response mediated by circulating immunoglobulin')
TissueDGE_results <- TissueDGE_results[TissueDGE_results$Term %in% terms, ]
TissueDGE_results$cluster <- factor(TissueDGE_results$cluster, 
                             levels=c(levels(seu$combo)))
TissueDGE_results <- TissueDGE_results %>% drop_na() # gets rid of any rows that pertained to cells outside of our B cell subset
TissueDGE_results$pval <- as.numeric(TissueDGE_results$pval)
TissueDGE_results$tissue <- sub("^[^_]*_", "", TissueDGE_results$cluster)
TissueDGE_results$cluster <- sub("_[^_]+$", "", TissueDGE_results$cluster)
TissueDGE_results$Fold_enrichment_I <- ifelse(TissueDGE_results$tissue == 'I', TissueDGE_results$Fold_enrichment, NA)
TissueDGE_results$Fold_enrichment_J <- ifelse(TissueDGE_results$tissue == 'J', TissueDGE_results$Fold_enrichment, NA)
TissueDGE_results$Term <- factor(TissueDGE_results$Term,
                          levels = (c('B cell activation',
                                      'antigen processing and presentation',
                                      'antigen processing and presentation of peptide or polysaccharide antigen via MHC class II',
                                      'T cell activation involved in immune response',
                                      'humoral immune response',
                                      'humoral immune response mediated by circulating immunoglobulin')))
ggplot(TissueDGE_results) +
  geom_point(aes(x = cluster, 
                 y = Term,
                 size = Fold_enrichment_J,
                 color = pval)) +
  scale_colour_gradient2(low="grey90", mid="blue", high="blue4", 
                         limits=c(0, 0.05),
                         midpoint = 0.015) +
  theme_bw() + 
  theme(axis.text.x = element_text(angle=90, vjust=.5, hjust=1)) +
  ...
  geom_point(aes(x = cluster, 
                 y = Term,
                 size = Fold_enrichment_I,
                 color = pval)) +
  scale_colour_gradient2(low="grey90", mid="red", high="red4", 
                         limits=c(0, 0.05),
                         midpoint = 0.015)
