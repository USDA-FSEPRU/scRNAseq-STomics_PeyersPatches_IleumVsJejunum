...
## Read in manual annotation assignments and apply to Seurat object:
manI2 <- read.csv('/home/Jayne.Wiarda/SI_PP_SC_ST/ManualAnnotation_Spatial/Region_I2.csv')
manI2$Barcode <- paste('I2', manI2$Barcode, sep = '_')
manI3 <- read.csv('/home/Jayne.Wiarda/SI_PP_SC_ST/ManualAnnotation_Spatial/Region_I3.csv')
manI3$Barcode <- paste('I3', manI3$Barcode, sep = '_')
manI4 <- read.csv('/home/Jayne.Wiarda/SI_PP_SC_ST/ManualAnnotation_Spatial/Region_I4.csv')
manI4$Barcode <- paste('I4', manI4$Barcode, sep = '_')
manJ2 <- read.csv('/home/Jayne.Wiarda/SI_PP_SC_ST/ManualAnnotation_Spatial/Region_J2.csv')
manJ2$Barcode <- paste('J2', manJ2$Barcode, sep = '_')
manJ3 <- read.csv('/home/Jayne.Wiarda/SI_PP_SC_ST/ManualAnnotation_Spatial/Region_J3.csv')
manJ3$Barcode <- paste('J3', manJ3$Barcode, sep = '_')
manJ4 <- read.csv('/home/Jayne.Wiarda/SI_PP_SC_ST/ManualAnnotation_Spatial/Region_J4.csv')
manJ4$Barcode <- paste('J4', manJ4$Barcode, sep = '_')
man <- do.call("rbind", list(manI2, manI3, manI4, manJ2, manJ3, manJ4))
rm(manI2, manI3, manI4, manJ2, manJ3, manJ4)
colnames(man)[colnames(man) == "Region"] ="Region_ManAnn"
man$Region_ManAnn[man$Region_ManAnn == 'Villi'] <- 'Villus' # accidently labeled annotation as villi in some samples, so switch to villus to be uniform
head(man)

dim(man)
keep <- colnames(seu)
man <- man[man$Barcode %in% keep,]
dim(man)
table(colnames(seu) == man$Barcode) # since all barcodes are in correct order (all come back as TRUE), no need to rearrange. If any FALSE come back, need to rearrange row order in man to match colnames(st)
man <- transform(man, match = ifelse(Region_ManAnn==Region_Clust, 'TRUE', 'FALSE'))
table(man$match)

rownames(man) <- man$Barcode
man$Barcode <- NULL
head(man)
seu <- AddMetaData(seu, man)
