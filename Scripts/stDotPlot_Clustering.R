...
# Identify spots that have same (TRUE) versus different (FALSE) annotation assignments between clustering and manual annotation methods
man <- data.frame(seu$Region_Clust, seu$Region_ManAnn)
man <- transform(man, match = ifelse(seu.Region_ManAnn==seu.Region_Clust, 'TRUE', 'FALSE'))
table(man$match)
head(man)

man$seu.Region_Clust <- NULL
man$seu.Region_ManAnn <- NULL
head(man)
seu <- AddMetaData(seu, man)

SpatialDimPlot(seu, alpha = c(0, 0), ncol = 3) & NoLegend() # set alpha to zero to just show H&E images

cols <- c('mediumseagreen', 'goldenrod2', 'slateblue', 'chocolate',  'burlywood4')
Idents(seu) <- seu$Region_ManAnn
SpatialDimPlot(seu, ncol = 3) & scale_fill_manual(values=cols)

cols <- c('burlywood4', 'slateblue','mediumseagreen', 'goldenrod2', 'chocolate')
Idents(seu) <- seu$Region_Clust
SpatialDimPlot(seu, ncol = 3) & scale_fill_manual(values=cols)

cols <- c('grey70', 'red')
Idents(seu) <- seu$match
SpatialDimPlot(seu, ncol = 3) & scale_fill_manual(values=cols) 

# see levels of consensus for annotations in each tissue section:
table(seu$match, seu$SampleID)

# Also plot some genes to see similar expression profiles across annotations:
DefaultAssay(seu) <- 'SCT'
Idents(seu) <- seu$Region_Clust
DotPlot(seu, features = c('PIGR', 'OLFM4', 'REG4', 'MUC2', 'CHGA',
                          'JCHAIN', 'XBP1',
                          'FABP2', 'SI', 'GUCA2A', 
                          'CD3E', 'CD3G', 'CD4', 'CD8B', 'TRDC', 'CD2', 'CD8A', 'CCL5', 'ITGAE', 'GZMB', 'CCR9', 'KLRK1',
                          'SELL', 'SLA-DQB1', 'SIRPA', 'CD68', 
                          'PTPRC', 'CCR7', 'S1PR1', 'ICOS', 'BLK',
                          'CD79A', 'CD19', 'MS4A1', 'IRF4', 'CD69', 'CD83', 'CD40', 'AICDA', 'PCLAF', 'TOP2A', 
                          'PECAM1', 'COL1A1', 'MYH11', 'ACTG2',
                          'RPL5'), # add last gene so we scale to max dot size of 100%...end up cropping this gene out
        cols = c('gold', 'red3'),
        col.min = -1.75,
        col.max = 1.75) + RotatedAxis()
