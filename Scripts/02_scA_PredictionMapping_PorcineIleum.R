...
#We've updated our gene nomencluture for the ileum/jejunum scRNA-seq dataset compared to the previous ileum scRNA-seq dataset we will use as a reference. To make gene names directly comparable, we will convert the gene names of the query data. We choose to change the query and not the reference gene names because otherwise we would have to reconstruct the reference Seurat object. We already have to do so for the query anyways.

#Load keys for gene name conversions.

#Load information for old gene nomencluture, including final name used and corresponding Ensembl ID:

pigGenesOld <- read_excel('/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/QC/UnfilteredGeneInfo.xlsx') # load gene nomenclature used for previous ileum scRNA-seq dataset
colnames(pigGenesOld)
pigGenesOld <- pigGenesOld[, c('EnsemblID', 'Name')] # subset to only columns needed
...
colnames(pigGenesOld) <- paste0(colnames(pigGenesOld), '_old') # append the suffix '_old' to each column
colnames(pigGenesOld)

#Load information for new gene nomencluture, including final name used and corresponding Ensembl ID:

pigGenesNew <- read_excel('/home/Jayne.Wiarda/scRNAseqEpSI/UpdatedGeneNameListForSus97GTF.xlsx') # read in file with an updated gene symbol annotation for Sus scrofa v97 annotation build
colnames(pigGenesNew)
pigGenesNew <- pigGenesNew[, c('ENSID', 'FinalAnnot')] # subset to only columns needed
pigGenesNew$FinalAnnot <-gsub("_", "-", pigGenesNew$FinalAnnot) # replace all underscores with dashes since this occurred when processing data into Seurat
colnames(pigGenesNew) <- paste0(colnames(pigGenesNew), '_new') # append the suffix '_new' to each column
colnames(pigGenesNew)

#Merge together new and old annotations based on shared Ensembl IDs:

identical(nrow(pigGenesNew), nrow(pigGenesOld)) # make sure the number of rows (genes) between new and old annotations are equal
pigGenes <- merge(pigGenesOld, pigGenesNew, by.x = 'EnsemblID_old', by.y = 'ENSID_new')
colnames(pigGenes)
colnames(pigGenes) <- c('EnsemblID', 'Name_old', 'Name_new')
colnames(pigGenes)
setdiff(rownames(seu), pigGenes$Name_new) # first make extra sure we don't have any unique names in our Seurat object compared to annotation file
# we see there is a naming fluke with ISG12(A)-ENSSSCG00000035297 that occurred when creating custom annotation file, so we need to manually convert the one entry. We made sure this was not an issue with any other genes and that all other genes were accurately converted.
pigGenes[pigGenes == "ISG12(A)"] <- "ISG12(A)-ENSSSCG00000035297"
rm(pigGenesOld, pigGenesNew)

#Make new Seurat object of query data with old gene names:

querycounts <- GetAssayData(object = seu, slot = "counts") # extract raw counts from Seurat object

pigGenes <- pigGenes[pigGenes$Name_new %in% rownames(seu), ] # slim down to only genes in our dataset
pigGenes <- pigGenes %>% arrange(factor(Name_new, levels = rownames(querycounts))) # arrange pigGenes in same order as Seurat object
rownames(querycounts) <- pigGenes$Name_old # change pig gene names to updated gene annotations
setdiff(rownames(querycounts), pigGenes$Name_new) # should find plenty of differences in gene names here
setdiff(rownames(querycounts), pigGenes$Name_old) # should find no differences in gene names here

seu <- CreateSeuratObject(counts = querycounts)
seu
...
ref <- LoadH5Seurat('/home/Jayne.Wiarda/scRNAseqIleumAtlas/Ileum/DataDepo/Ileum_AllCells.h5seurat')
ref

# change to annotation nomenclature used in publication:
Idents(ref) <- ref$cellID
ref <- RenameIdents(ref, 'Activated CD4 ab T cells' = 'Non-naive CD4 ab T cells')
ref <- RenameIdents(ref, 'Activated CD8 ab T cells' = 'Non-naive CD8 ab T cells')
ref <- RenameIdents(ref, 'Activated gd T cells' = 'Non-naive gd T cells')
ref <- RenameIdents(ref, 'Activated group 1 ILCs' = 'Non-naive group 1 ILCs')
ref$cellID <- Idents(ref)

DimPlot(ref, reduction = 'tsne', label = TRUE)
...
for(i in 1:length(seu.list)) {
  ...
  predictions <- TransferData(anchorset = anchors, 
                              refdata = list(cell_type = ref$cellID), 
                              dims = 1:30,
...
write_xlsx(CellTypePredictions, '/home/Jayne.Wiarda/SI_PP_SC_ST/MappingPrediction/WiardaIleumAtlas_CellTypePredictions.xlsx')
write_xlsx(MappingScores, '/home/Jayne.Wiarda/SI_PP_SC_ST/MappingPrediction/WiardaIleumAtlas_MappingScores.xlsx')
...
VlnPlot(seu, split.by = 'orig.ident', features = 'MappingScores') # any major differences in mapping scores between ileum and jejunum data?
...
FeaturePlot(seu, # plot all prediction scores
            features = c('prediction.score.Cycling.B.cells', 'prediction.score.Epithelial.cells',
                         'prediction.score.Group.3.ILCs', 'prediction.score.Cytotoxic.gd.T.cells',
                         'prediction.score.Follicular.CD4.ab.T.cells', 'prediction.score.Dendritic.cells'),
            reduction = 'umap', # change to 'tsne' to overlay onto t-SNE plot
            ncol = 2) & 
  scale_color_gradientn( colours = c('grey90', 'blue3'),  limits = c(0, 1)) & 
  NoAxes() & NoLegend() 

VlnPlot(seu, split.by = 'orig.ident', features = 'prediction.score.max') # any major differences in prediction scores between ileum and jejunum data? 
...
DimPlot(seu, group.by = 'predicted.id', reduction = 'umap', label = TRUE) # plot predicted cell IDs (based on highest prediction scores)
